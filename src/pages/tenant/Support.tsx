...
export default function Support() {
  ...
  const items = [
    ...
    { icon: Mail, label: 'Email Support', desc: 'support@urbangravity.com' },
    { icon: Phone, label: 'Call Us', desc: '+234 800 000 0000' },
    { icon: Shield, label: 'Safety Tips', desc: 'Stay safe while renting' },
    { icon: FileText, label: 'Terms & Conditions', desc: 'Read our terms of service' },
  ];
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <Card key={item.label} hover className="flex items-center gap-4 cursor-pointer">
            ...
            <div><p className="font-bold text-sm text-[#1A1A1A]">{item.label}</p><p className="text-xs text-[#666]">{item.desc}</p></div>
          </Card>
        ))}
      </div>
    </div>
